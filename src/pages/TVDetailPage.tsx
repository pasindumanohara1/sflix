...
export default function TVDetailPage() {
  ...
  useEffect(() => {
    ...
    tmdb.getTvById(id)
      .then((d) => {
        setData(d);
        if (d.seasons?.length) setSeason(d.seasons[0].season_number || 1);
      })
      .catch(() => setData(null))
  ...
  return (
    <Layout>
      <Helmet>
        <title>{title} - SFlix | Watch TV Series Online Free</title>
        <meta name="description" content={`Watch ${title} online free in HD quality at SFlix. Stream the latest TV shows and series.`} />
      </Helmet>
          ...
          <div className="detail-info">
            ...
            <div className="detail-stats">
              ...
              {data.episode_run_time?.[0] && (
                <span className="detail-duration">
                  <i className="far fa-clock" /> {data.episode_run_time[0]} min
                </span>
              )}
            </div>
            ...
            <div className="detail-servers">
              {SERVERS.slice(0, 5).map((server) => (
                <Link
                  key={server.id}
                  to={`/watch/${slugForWatch}?server=${server.id}&season=${season}&episode=${episode}`}
            ...
            <div className="detail-metadata">
              {data.first_air_date && (
                <div className="detail-meta-row">
                  ...
                  <span className="meta-value">{data.first_air_date}</span>
                </div>
              )}
              {genres.length > 0 && (
                <div className="detail-meta-row">
              ...
              {data.episode_run_time?.[0] && (
                <div className="detail-meta-row">
                  ...
                  <span className="meta-value">{data.episode_run_time[0]} min</span>
                </div>
              )}
              ...
              {data.number_of_seasons && (
                <div className="detail-meta-row">
                  ...
                  <span className="meta-value">{data.number_of_seasons}</span>
                </div>
              )}
            </div>
      ...
      <style>{`
        ...
        .server-btn i { color: var(--accent-blue); font-size: 0.75rem; }
        ...
        .detail-metadata { margin-bottom: 1rem; }
        ...
        .meta-label { color: var(--text-muted); flex-shrink: 0; min-width: 70px; }
        ...
        @media (max-width: 768px) {
          ...
          .season-selector { text-align: left; }
        }
